...
fn derive_to_value_without_context(
    mut input: DeriveInput,
    trait_path: Path,
    output_type_name: Ident,
    bind_style: BindStyle,
    ...
    mut call_from: impl FnMut(&BindingInfo) -> TokenStream,
    mut call_to: impl FnMut(&BindingInfo) -> TokenStream,
    mut call_to_without_context: impl FnMut(&BindingInfo) -> TokenStream,
    // Returns a tokenstream of the form:
    // fn from_function_syntax(foobar) -> Baz {
    //     #first_arg
    // }
    ...
    //     #second_arg
    // }
    mut trait_impl: impl FnMut(TokenStream, TokenStream, TokenStream) -> TokenStream,
) -> TokenStream {
    let name = &input.ident;
    ...
    let (to_body, to_body_without_context, from_body) = if params.is_empty() {
        let mut s = synstructure::Structure::new(&input);
        s.variants_mut().iter_mut().for_each(|v| {
            v.bind_with(|_| bind_style);
        });
        ...
        let to_body_without_context = if moves {
            quote! { Ok(self) }
        } else {
            quote! { Ok(std::clone::Clone::clone(self)) }
        };
        ...
        (to_body, to_body_without_context, from_body)
    } else {
        let to_body = cg::fmap_match(&input, bind_style, |binding| {
            let attrs = binding_attrs(&binding);
            assert!(
        ...
        let to_body_without_context = cg::fmap_match(&input, bind_style, |binding| {
            let attrs = binding_attrs(&binding);
            assert!(
                !attrs.no_field_bound,
                "It doesn't make sense on a generic implementation"
            ...
            call_to_without_context(&binding)
        });
        ...
        let to_body_without_context = quote! {
            match #self_ {
                #to_body_without_context
            }
        };
    ...
    let impl_ = trait_impl(from_body, to_body, to_body_without_context);
...
pub fn derive(input: DeriveInput) -> TokenStream {
    let trait_impl = |from_body, to_body, to_body_without_context| {
        quote! {
             #[inline]
    ...
    derive_to_value_without_context(
        input,
        ...
        |binding| quote!(crate::values::computed::ToComputedValue::to_computed_value_without_context(#binding)),
        trait_impl,
    )
}
