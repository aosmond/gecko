...
VectorImage::GetFrameInternal(const IntSize& aSize,
  ...
  // TODO(aosmond): We need to include the region to support caching partial
  // recordings.
  uint32_t flags = aFlags;
  if (aRegion) {
    flags |= FLAG_BYPASS_SURFACE_CACHE;
  }
  ...
  Tie(sourceSurface, decodeSize) =
      LookupCachedSurface(aSize, aSVGContext, whichFrame, flags);
  if (sourceSurface) {
