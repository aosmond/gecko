...
SourceSurfaceBlobImage::SourceSurfaceBlobImage(
    image::VectorImage* aImage, const Maybe<SVGImageContext>& aSVGContext,
    const IntSize& aSize, uint32_t aImageFlags)
    : mImage(aImage),
...
SourceSurfaceBlobImage::~SourceSurfaceBlobImage() {
  ...
  NS_ReleaseOnMainThread("SourceSurfaceBlobImage::mImage", mImage.forget());
...
/* static */ void SourceSurfaceBlobImage::DestroyKeys(
    const AutoTArray<ImageKeyData, 1>& aKeys) {
...
Maybe<wr::BlobImageKey> SourceSurfaceBlobImage::UpdateKey(
  ...
  while (i > 0) {
    ...
    if (entry.mManager->IsDestroyed()) {
      ...
      // to rerecord it.
      auto newEntry = RecordDrawing(
          aManager, aResources, ownsKey ? Some(entry.mBlobKey) : Nothing());
      if (!newEntry) {
        if (ownsKey) {
...
Maybe<SourceSurfaceBlobImage::ImageKeyData>
SourceSurfaceBlobImage::RecordDrawing(WebRenderLayerManager* aManager,
                                      wr::IpcResourceUpdateQueue& aResources,
                                      Maybe<wr::BlobImageKey> aBlobKey) {
  MOZ_ASSERT(!aManager->IsDestroyed());

  if (mImage->mIsDrawing) {
    return Nothing();
  }
  ...
  {
    auto* documentWrapper = mImage->mSVGDocumentWrapper.get();
    ...
    SVGDrawingParameters params(
        ...
        SamplingFilter::POINT, mSVGContext,
        documentWrapper->GetCurrentTimeAsFloat(), mImageFlags, 1.0);

    AutoRestoreSVGState autoRestore(params, documentWrapper,
                                    mImage->mIsDrawing, contextPaint);

    RefPtr<gfxDrawable> svgDrawable = mImage->CreateSVGDrawable(params);
    documentWrapper->UpdateViewportBounds(params.viewportSize);
    documentWrapper->FlushImageTransformInvalidation();
  ...
  return Some(ImageKeyData(aManager, key, std::move(fonts),
