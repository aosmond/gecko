...
nsresult ImageEncoder::VerifyParameters(uint32_t aLength, uint32_t aWidth,
  ...
  CheckedInt32 dataLen = CheckedInt32(aStride) * CheckedInt32(aHeight);
...
DataSurfaceFlags ImageEncoder::ToSurfaceFlags(uint32_t aInputFormat) const {
  switch (aInputFormat) {
    case INPUT_FORMAT_RGBA:
      // The encoders assume that RGBA implies unpremultiplied. This only
      // applies for the JS APIs.
...
nsresult ImageEncoder::PrepareRowPipeline(SurfaceFormat aIn,
                                          ...
                                          SurfaceFormat aOut,
                                          const IntSize& aSize) {
  // We could go through some heroics here to avoid extra passes, such as when
  // the swizzle is just a copy and we copy again to the encoding buffer, or
  // when the swizzle could be performed by premultiplication. But performance
  // for the encoders is not critical.
  mSwizzleFn = SwizzleRow(aIn, aOut);
  if (NS_WARN_IF(!mSwizzleFn)) {
    return NS_ERROR_FAILURE;
  }

  // We only need to consider premultiplication for surfaces with alpha.
  if (!IsOpaque(aIn)) {
    if (aInFlags & DataSurfaceFlags::UNPREMULTIPLIED_ALPHA) {
      // Surface is unpremultiplied. If we are dropping the alpha, we actually
      // need to perform premultiplication first. If we are keeping the alpha,
      // we don't need to do anything.
      if (IsOpaque(aOut)) {
        mPremultiplyFn = PremultiplyRow(aIn, aIn);
        if (NS_WARN_IF(!mPremultiplyFn)) {
          return NS_ERROR_FAILURE;
        }
      }
    } else {
      // Surface is premultiplied. If we are dropping the alpha, we don't need
      // to do anything. If we are keeping the alpha, we need to reverse the
