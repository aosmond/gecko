...
static void cmyk_convert_rgba(uint32_t* aPixelBlock, int32_t aBlockSize);
...
WriteState nsJPEGDecoder::OutputScanlines() {
  auto result = mPipe.WritePixelBlocks<uint32_t>(
      [&](uint32_t* aPixelBlock, int32_t aBlockSize) {
        JSAMPROW sampleRow = (JSAMPROW)aPixelBlock;
        if (mInfo.out_color_space == JCS_GRAYSCALE) {
          // Grayscale will only output 1 byte per pixel. We only use this when
          // we need to apply a color transform, and said transform is also in
          // grayscale. If we place the grayscale pixels at the end of the
          // buffer, that leaves sufficient room to do the transform in place.
          MOZ_ASSERT(mTransform);
          sampleRow += 3 * mInfo.output_width;
        }
        ...
        if (mInfo.out_color_space == JCS_GRAYSCALE) {
          ...
          MOZ_ASSERT(mTransform);
          qcms_transform_data(mTransform, sampleRow, aPixelBlock, mInfo.output_width);
        }

        if (mInfo.out_color_space == JCS_CMYK) {
          // Convert from CMYK to BGRA
          cmyk_convert_rgba(aPixelBlock, aBlockSize);
        }
...
static void cmyk_convert_rgba(uint32_t* aPixelBlock, int32_t aBlockSize) {
  uint8_t* data = reinterpret_cast<uint8_t*>(aPixelBlock);

  for (int32_t i = 0; i < aBlockSize; ++i) {
    ...
    const uint32_t iC = data[0];
    const uint32_t iM = data[1];
    const uint32_t iY = data[2];
    const uint32_t iK = data[3];
#if MOZ_BIG_ENDIAN
    data[0] = 0xFF;           // Alpha
    data[1] = iC * iK / 255;  // Red
    data[2] = iM * iK / 255;  // Green
    data[3] = iY * iK / 255;  // Blue
#else
    data[0] = iY * iK / 255;  // Blue
    data[1] = iM * iK / 255;  // Green
    data[2] = iC * iK / 255;  // Red
    data[3] = 0xFF;           // Alpha
#endif
