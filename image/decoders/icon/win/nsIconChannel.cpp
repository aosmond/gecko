...
void
nsIconChannel::IconAsyncOpenTask::Run()
{
  HICON hIcon = nullptr;
  nsresult rv =
...
nsIconChannel::Open(nsIInputStream** _retval) {
  HICON hIcon = nullptr;
  nsresult rv = GetHIcon(/* aNonBlocking */ false, &hIcon);
  if (NS_FAILED(rv)) {
    return rv;
  }

  return MakeInputStream(_retval, /* aNonBlocking */ false, hIcon);
}
...
void
nsIconChannel::OnAsyncError(nsresult aStatus)
{
  OnStartRequest(this, mListenerContext);
  OnStopRequest(this, mListenerContext, aStatus);
}

void
nsIconChannel::FinishAsyncOpen(HICON aIcon, nsresult aStatus)
{
...
nsIconChannel::AsyncOpen(nsIStreamListener* aListener, nsISupports* ctxt) {
  ...
  nsresult rv = EnsurePipeCreated(/* aIconSize */ 0, /* aNonBlocking */ true);
  if (NS_FAILED(rv)) {
    mCallbacks = nullptr;
    return rv;
  }
...
nsresult nsIconChannel::GetHIconFromFile(bool aNonBlocking, HICON* hIcon) {
  ...
  if (aNonBlocking) {
    RefPtr<IconAsyncOpenTask> task =
      new IconAsyncOpenTask(this, mListenerTarget, std::move(localFile),
                            filePath, infoFlags);
...
nsresult nsIconChannel::GetStockHIcon(nsIMozIconURI* aIconURI,
  ...
  if (aNonBlocking) {
    nsCOMPtr<nsIRunnable> task = NewRunnableMethod<HICON, nsresult>(
      "nsIconChannel::FinishAsyncOpen", this,
      &nsIconChannel::FinishAsyncOpen, sii.hIcon, rv);
...
nsresult nsIconChannel::EnsurePipeCreated(uint32_t aIconSize,
  ...
  return NS_NewPipe(getter_AddRefs(mInputStream),
                    getter_AddRefs(mOutputStream),
                    aIconSize,
                    aIconSize > 0 ? aIconSize : UINT32_MAX,
                    aNonBlocking);
}
