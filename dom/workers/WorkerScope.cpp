...
int32_t DedicatedWorkerGlobalScope::RequestAnimationFrame(
  ...
  if (mWorkerPrivate->WindowID() == UINT64_MAX) {
    aError.ThrowNotSupportedError("Worker has no window");
    return 0;
  }
  ...
  if (!mVsyncChild) {
    ...
    if (!bgChild || !mVsyncChild->Initialize(mWorkerPrivate) ||
      ...
      aError.ThrowNotSupportedError(
          "Worker failed to register for vsync events");
      return 0;
    }
  }
...
void DedicatedWorkerGlobalScope::CancelAnimationFrame(int32_t aHandle,
  ...
  // If we cancelled the last callback, then we will disable observing vsync on
  // the next tick, if no more are issued.
