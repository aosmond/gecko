...
VideoFrameCallbackMetadata* HTMLVideoElement::GetVideoFrameCallbackMetadata() {
      ...
      */
        ...
        VideoFrameCallbackMetadata* md = new VideoFrameCallbackMetadata(
                                         this,
                                         mdPresentationTime,
                                         mdExpectedDisplayTime,
                                         mdWidth,
                                         mdHeight,
                                         mdMediaTime,
                                         mdPresentedFrames,
                                         mdProcessingDuration,
                                         mdCaptureTime,
                                         mdReceiveTime,
                                         mdRtpTimestamp);

        printf_stderr("\n%s:%s:%d AZ - HTMLVideoElement generated VideoFrameCallbackMetadata with:\n"
                      "presentationTime: %f \n"
        ...
        VideoFrameCallbackMetadata* md = new VideoFrameCallbackMetadata(
                                       this,
                                       CurrentTime(), // presentation time (when submitted)
                                       expectedDisplayTime, // expected display time

                                       //345,
                                       //el->Height(), // media pixels
                                       videoInfo.mDisplay.Width(), // media pixels
                                       videoInfo.mDisplay.Height(), // media pixels
                                       CurrentTime(), // "mediaTime" - presentation timestamp of frame to
                                          // be presented

                                       MozPresentedFrames() + 1, // presented frames
                                       0.1, // processing duration

                                       ts, // capture time. SHOULD be present for WebRTC or getUserMedia applications, and absent otherwise
                                       ts, // receive time " " "
                                       0); // rtpTimestamp " " "
                                       */
       /*
        printf_stderr("%s:%s:%d AZ - HTMLVideoElement generated VideoFrameCallbackMetadata with:\n"
                      "presentationTime: %f \n"
                      "expectedDisplayTime: %f \n"
                      "width: %d \n"
                      "height: %d \n"
...
void HTMLVideoElement::TakeVideoFrameRequestCallbacks(nsTArray<VideoFrameRequest>& aCallbacks) {
...
unsigned long HTMLVideoElement::RequestVideoFrameCallback(VideoFrameRequestCallback& aCallback) {
  printf_stderr("\n");
  printf_stderr("\n");
  printf_stderr("\n");
