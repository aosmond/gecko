...
void HTMLVideoElement::TakeVideoFrameRequestCallbacks(
  ...
  // If given, the capture time is either an NTP WebRTC timestamp (remote), or a
  // high resolution timestamp. For the former, we need to convert into a DOM
  // timestamp using the WebRTC session's timestamp maker. For the latter, we
  // need to adjust its resolution using the same mechanism as the refresh
  // driver.
  ...
  } else if (selected->mWebrtcCaptureTime.is<TimeStamp>()) {
  ...
  // If given, the receive time is a "realtime" WebRTC timestamp. We need to
  // convert from that into a DOM timestamp using the WebRTC session's timestamp
  // maker.
  if (selected->mWebrtcReceiveTimeUs) {
    auto receiveTimeReal = *selected->mWebrtcReceiveTimeUs;
    if (mSelectedVideoStreamTrack && receiveTimeReal > 0) {
      if (const auto* timestampMaker =
              mSelectedVideoStreamTrack->GetSource().GetTimestampMaker()) {
        aMd.mReceiveTime.Construct(
            RTCStatsTimestamp::FromRealtime(
                *timestampMaker, webrtc::Timestamp::Micros(receiveTimeReal))
                .ToDom());
      }
    }
  }
