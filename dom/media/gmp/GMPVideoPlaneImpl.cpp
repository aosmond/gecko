...
GMPErr GMPPlaneImpl::MaybeResize(int32_t aNewSize) {
  if (!mArrayBuffer.SetCapacity(aNewSize, fallible)) {
    return GMPAllocErr;
  }
...
const uint8_t* GMPPlaneImpl::Buffer() const { return mArrayBuffer.Elements(); }
uint8_t* GMPPlaneImpl::Buffer() { return mArrayBuffer.Elements(); }
