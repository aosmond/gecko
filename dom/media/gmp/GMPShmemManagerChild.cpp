...
GMPShmemManagerChild::~GMPShmemManagerChild() { MgrPurgeShmems(); }

bool GMPShmemManagerChild::MgrTakeShmem(size_t aCapacity, ipc::Shmem* aShmem) {
  GMP_LOG_DEBUG(
      "GMPShmemManagerChild::MgrTakeShmem(this=%p capacity=%zu) "
      "bufferSizes={%s}",
...
bool GMPShmemManagerChild::MgrHasShmem(size_t aCapacity) {
...
void GMPShmemManagerChild::MgrForgetShmem(size_t aCapacity) {
...
void GMPShmemManagerChild::MgrGiveShmem(ipc::Shmem&& aShmem) {
  size_t sz = aShmem.Size<uint8_t>();
  mBuffers.AppendElement(std::move(aShmem));
  GMP_LOG_DEBUG(
      "GMPShmemManagerChild::MgrGiveBuffer(this=%p, capacity=%zu) "
      "bufferSizes={%s}",
      this, sz, ToString(mBuffers).get());
}

void GMPShmemManagerChild::MgrPurgeShmems() {
