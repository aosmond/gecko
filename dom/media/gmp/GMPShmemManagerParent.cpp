...
bool GMPShmemManagerParent::PurgeShmems() {
  GMP_LOG_DEBUG(
      "GMPShmemManagerParent::PurgeShmems(this=%p) frame_size=%zu "
...
bool GMPShmemManagerParent::EnsureSufficientShmems(size_t aCapacity) {
  GMP_LOG_DEBUG(
      "GMPShmemManagerParent::EnsureSufficientShmems(this=%p) size=%zu "
  ...
  while (mShmemsActive < mShmemLimit) {
    ipc::Shmem shmem;
    if (!ProtoAllocShmem(mBufferSize, &shmem)) {
      return false;
    }
    if (!ProtoGiveShmem(std::move(shmem))) {
...
bool GMPShmemManagerParent::IncreaseShmemPoolSize() {
  ...
  EnsureSufficientShmems(mBufferSize);
  return true;
}
