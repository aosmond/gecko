...
void GMPVideoEncoder::InitComplete(GMPVideoEncoderProxy* aGMP,
  ...
  GMPErr err = aGMP->InitEncode(codec, codecSpecific, this,
...
RefPtr<MediaDataEncoder::EncodePromise> GMPVideoEncoder::Encode(
  ...
  CheckedInt32 yBufSize = CheckedInt32(yuv->mYStride) * ySize.height;
  MOZ_RELEASE_ASSERT(yBufSize.isValid());

  CheckedInt32 cbCrBufSize = CheckedInt32(yuv->mCbCrStride) * cbCrSize.height;
  ...
  err = frame->CreateFrame(
      yBufSize.value(), yuv->mYChannel, cbCrBufSize.value(), yuv->mCbChannel,
      cbCrBufSize.value(), yuv->mCrChannel, ySize.width, ySize.height,
      yuv->mYStride, yuv->mCbCrStride, yuv->mCbCrStride);
  if (NS_WARN_IF(err != GMPNoErr)) {
    return EncodePromise::CreateAndReject(NS_ERROR_DOM_MEDIA_FATAL_ERR,
                                          __func__);
  }
