...
def fetch_url_for_cdms(cdms, urlParams):
    ...
    for cdm in cdms:
        if "fileName" in cdm:
            ...
            cdm["fileNameAlternate"] = alternateUrl
    return any_version
...
def fetch_data_for_cdms(cdms, urlParams):
    for cdm in cdms:
        if "fileName" in cdm:
            ...
            if "fileNameAlternate" in cdm:
                cdm["fileUrlAlternate"] = cdm["fileNameAlternate"].format_map(urlParams)
                altresponse = requests.get(cdm["fileUrlAlternate"])
                altresponse.raise_for_status()
                althash = hashlib.sha512(altresponse.content).hexdigest()
                if cdm["hashValue"] != althash:
                    raise Exception("Different hash for primary and alternate URLs")
