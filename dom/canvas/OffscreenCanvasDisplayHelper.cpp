...
void OffscreenCanvasDisplayHelper::UpdateContext(
  ...
  if (auto* cm = gfx::CanvasManagerChild::Get()) {
    mContextManagerId.emplace(cm->Id());
  }
...
bool OffscreenCanvasDisplayHelper::CommitFrameToCompositor(
  ...
  if (mData.mOwnerId.isSome() && !mData.mDoPaintCallbacks) {
  ...
  if (hasRemoteTextureDesc) {
    ...
    mFrontBufferTextureId = Some(textureDesc.textureId());
...
OffscreenCanvasDisplayHelper::GetSurfaceSnapshot() {
  ...
  Maybe<layers::RemoteTextureId> textureId;

  {
    ...
    textureId = mFrontBufferTextureId;
  }
  ...
  if (managerId && childId) {
    ...
    return gfx::CanvasManagerChild::Get()->GetSnapshot(
        managerId.value(), childId.value(), ownerId, textureId,
