    ...
    while (forPrincipal > maxContextsPerPrincipal) {
      ...
      for (const auto& context : sLru) {
        if (context->mPrincipalKey == mPrincipalKey) {
          ...
          context->LoseContextLocked(webgl::ContextLossReason::None, lock);
          forPrincipal -= 1;
          break;
        }
      }
...
void WebGLContext::LoseContextLocked(const webgl::ContextLossReason reason,
                                     const StaticMutexAutoLock& aProofOfLock) {
