...
void Document::UpdateFrameRequestCallbackSchedulingState(
  ...
  printf_stderr(
      ...
      __FILE__, __func__, __LINE__, WouldScheduleFrameRequestCallbacks(),
      scheduleAFs, scheduleVFCs);
  ...
  if (!mRescheduledVFCs.IsEmpty()) {
    printf_stderr("\nAZ: %s:%s:%d rescheduling %zu delayed frame callbacks\n\n",
                  __FILE__, __func__, __LINE__, mRescheduledVFCs.Length());
  ...
  printf_stderr(
      "AZ: %s:%s:%d (throttled) VideoFrameRequests before:: (%zu) %zu\n",
  ...
  if (scheduleVFCs != mVideoFrameCallbacksScheduled) {
    printf_stderr("\nAZ: %s:%s:%d %s video frame callback\n\n", __FILE__,
                  __func__, __LINE__, scheduleVFCs ? "sched" : "revoke");
  ...
  printf_stderr(
      "AZ: %s:%s:%d pending VFCs after updating scheduling state: %zu\n",
      __FILE__, __func__, __LINE__, mPendingVFCs.Length());
}
...
void Document::NotifyVideoFrameCallbacks(HTMLVideoElement* aElement) {
  if (mPendingVFCs.IndexOf(aElement) != mPendingVFCs.NoIndex) {
    printf_stderr(
        "AZ: %s:%s:%d NOT Double adding HTMLVideoElement to pending "
        "callbacks!\n",
        __FILE__, __func__, __LINE__);
    return;
  }
...
void Document::DelayVideoFrameCallbacks(HTMLVideoElement* aElement) {
  ...
  if (mRescheduledVFCs.IndexOf(aElement) !=
      mRescheduledVFCs.NoIndex) {
        printf_stderr("AZ: %s:%s:%d NOT Double adding HTMLVideoElement to
  delayed callbacks!\n",
                      __FILE__, __func__, __LINE__);
        return;
  }
  ...
  */
  if (mRescheduledVFCs.IndexOf(aElement) != mRescheduledVFCs.NoIndex) {
    printf_stderr(
        "AZ: %s:%s:%d NOT Double adding HTMLVideoElement to RESCHEDULED "
        "callbacks!\n",
        __FILE__, __func__, __LINE__);
    return;
  }
