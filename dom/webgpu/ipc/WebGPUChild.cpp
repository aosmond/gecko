...
void WebGPUChild::ActorDestroy(ActorDestroyReason) {
  ...
  for (const auto& targetIter : deviceMap) {
    ...
    ErrorResult rv;
    RefPtr<dom::Promise> promise = device->GetLost(rv);
    if (rv.Failed() || !promise) {
      continue;
    }

    auto info = MakeRefPtr<DeviceLostInfo>(device, u"WebGPUChild destroyed"_ns);

    // We have strong references to both the Device (through DeviceLostInfo) and
