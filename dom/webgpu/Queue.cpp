...
void Queue::CopyExternalImageToTexture(
  ...
  if (fromElement && !sfeResult.mCORSUsed) {
    auto& srcPrincipal = sfer.mPrincipal;
    nsIGlobalObject* owner = mParent->GetOwnerGlobal();
    nsIPrincipal* dstPrincipal = owner ? owner->PrincipalOrNull() : nullptr;
  ...
  const auto dstLength = dstStride * extent.height;
  if (!dstStride.isValid() || !dstLength.isValid()) {
    aRv.ThrowOperationError("Destination buffer dimensions invalid");
    return;
  }
  ...
  if (!ConvertImage(dstWidth, dstHeight, srcBegin, map.GetStride(),
                    gl::OriginPos::TopLeft, srcFormat, srcPremultiplied,
                    shmem.get<uint8_t>(), dstStride.value(), dstOriginPos,
                    dstFormat, aDestination.mPremultipliedAlpha, &wasTrivial)) {
