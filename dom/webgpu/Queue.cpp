...
void Queue::CopyExternalImageToTexture(
  ...
  if (srcFormat == dstFormat && srcOriginPos == dstOriginPos &&
      srcStride == dstStride.value() &&
      srcPremultiplied == aDestination.mPremultipliedAlpha) {
    for (uint32_t i = 0; i < dstHeight; ++i) {
      memcpy(dstBegin, srcBegin, srcStride);
      srcBegin += srcStride;
      dstBegin += srcStride;
    }
  } else if (!ConvertImage(dstWidth, dstHeight, srcBegin, srcStride,
                           srcOriginPos, srcFormat, srcPremultiplied, dstBegin,
