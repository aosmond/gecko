...
add_task(async function test_good_xml() {
  ...
  Assert.equal(addon.alternateURLs.length, 0);
  ...
  Assert.equal(addon.version, undefined);
  Assert.equal(addon.size, undefined);
  Assert.equal(addon.alternateURLs.length, 1);
  Assert.equal(addon.alternateURLs[0], "http://alt.example.com/test6.xpi");
  ...
  Assert.equal(addon.version, undefined);
  Assert.equal(addon.size, undefined);
  Assert.equal(addon.alternateURLs.length, 2);
  Assert.equal(addon.alternateURLs[0], "http://alt.example.com/test7.xpi");
  Assert.equal(addon.alternateURLs[1], "http://alt2.example.com/test7.xpi");
});
...
add_task(async function test_download_alt_missing() {
  try {
    let path = await ProductAddonChecker.downloadAddon({
      URL: root + "nofile.xpi",
      alternateURLs: [root + "nofile.xpi"],
    });
...
add_task(async function test_download_alt_works() {
  let path = await ProductAddonChecker.downloadAddon({
    URL: root + "nofile.xpi",
    alternateURLs: [root + "unsigned.xpi"],
    size: 452,
    hashFunction: "sha256",
    hashValue:
      "9b9abf7ddfc1a6d7ffc7e0247481dcc202363e4445ad3494fb22036f1698c7f3",
