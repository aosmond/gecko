...
async function downloadAndVerifyFileMirrors(addon, options) {
  ...
  throw new Error(`URLs for addon ${addon.id} exhausted`);
}
...
export const ProductAddonChecker = {
  ...
  async downloadAddon(addon, options = { httpsOnlyNoUpgrade: false }) {
    try {
      ...
      try {
        return await downloadAndVerifyFileMirrors(addon, options);
      } catch (_) {
        throw e;
      }
    }
