...
static void qcms_transform_data_template_lut_sse1(qcms_transform *transform,
                                                  unsigned char *src,
                                                  unsigned char *dest,
                                                  size_t length)
{
    ...
    vec_r = _mm_load_ss(&igtbl_r[src[R_INDEX]]);
    vec_g = _mm_load_ss(&igtbl_g[src[G_INDEX]]);
    vec_b = _mm_load_ss(&igtbl_b[src[B_INDEX]]);
    if (A_INDEX != 0xFF) {
        alpha = src[A_INDEX];
    }
    src += components;
