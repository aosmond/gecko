...
already_AddRefed<MacIOSurface> MacIOSurfaceRecycleAllocator::Allocate(
  ...
  if (!mSurfaces.IsEmpty() &&
      (mYSize != aYSize || mCbCrSize != aCbCrSize ||
       mChromeSubsampling != aChromeSubsampling ||
  ...
  for (auto& surf : mSurfaces) {
    ...
    MOZ_DIAGNOST_ASSERT(pixelFormat.isSome());
  ...
  // No match in the cache, update our cached parameters.
  mYSize = aYSize;
  mCbCrSize = aCbCrSize;
  mChromeSubsampling = aChromeSubsampling;
  mYUVColorSpace = aYUVColorSpace;
