...
bool CanvasTranslator::UnlockTexture(int64_t aTextureId, RemoteTextureId aId) {
  ...
  if (result->second.mDrawTarget &&
    ...
    if (aId.IsValid()) {
      PushRemoteTexture(data, result->second.mTextureLocked, aId, ownerId);
    } else {
...
bool CanvasTranslator::PushRemoteTexture(TextureData* aData, bool& aDataLocked,
                                         RemoteTextureId aId,
