...
void MLGDeviceD3D11::Clear(MLGRenderTarget* aRT, const gfx::DeviceColor& aColor) {
  MLGRenderTargetD3D11* rt = aRT->AsD3D11();
...
bool MLGDeviceD3D11::VerifyConstantBufferOffsetting() {
  ...
  // 2, and B from buffer 3.
  {
    ...
    *reinterpret_cast<DeviceColor*>(map.mData) = DeviceColor(1.0f, 0.2f, 0.3f, 1.0f);
    *reinterpret_cast<DeviceColor*>(map.mData + kConstantSize * kMinConstants) =
        DeviceColor(0.4f, 0.0f, 0.5f, 1.0f);
    *reinterpret_cast<DeviceColor*>(map.mData + (kConstantSize * kMinConstants) * 2) =
