...
already_AddRefed<DataSourceSurface> CanvasManagerChild::GetSnapshot(
    ...
    const Maybe<RemoteTextureId>& aTextureId, bool aPremultiply, bool aYFlip) {
  if (!CanSend()) {
    return nullptr;
  }

  SurfaceDescriptorShared desc;
  if (!SendGetSnapshot(aManagerId, aProtocolId, aOwnerId, aTextureId, &desc)) {
    return nullptr;
  }
  ...
  RefPtr<DataSourceSurface> surface = Factory::CreateWrappingDataSourceSurface(
      ...
      desiredFormat, DeallocateSnapshot, shmem.forget().take());
