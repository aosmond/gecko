                ...
                PrimitiveInstanceKind::Picture { pic_index, .. } => {
                    ...
                    let shadow_rect = match pic.raster_config {
                        Some(ref rc) => match rc.composite_mode {
                            ...
                            PictureCompositeMode::Filter(FilterOp::DropShadow(offset, ..)) => pic.local_rect.translate(&offset),
                            _ => LayoutRect::zero(),
                        }
                        None => {
                    ...
                    if prim_instance.is_chased() {
                        if pic.estimated_local_rect != pic.local_rect {
                            println!("\tsnapped from {:?} to {:?}", pic.estimated_local_rect, pic.local_rect);
                        }
                    }

                    (pic.raster_config.is_none(), false, pic.local_rect, shadow_rect)
                }
                _ => {
...
/// match the logic in the GLSL compute_snap_offset function.
...
/// Snap the given rect in device space if the transform is
...
pub fn get_snapped_rect<PixelSpace>(
    ...
    match transform_id.transform_kind() {
        TransformedRectKind::AxisAligned => {
            ...
            let snapped_device_rect = DeviceRect::from_points(&[
                device_rect.origin + top_left,
                device_rect.bottom_right() + bottom_right,
            ]);
