...
VertexInfo write_text_vertex(RectWithSize local_clip_rect,
    ...
    if (remove_subpx_offset) {
        ...
        switch (raster_space) {
            case RASTER_SCREEN: {
                ...
                device_snap_offset += floor(device_glyph_offset + snap_bias) - device_glyph_offset;
                break;
            }
            default: {
                ...
                vec2 device_text_offset = text_offset * task.device_pixel_scale;
                vec2 device_glyph_offset = glyph_offset * task.device_pixel_scale;
                device_snap_offset = floor(device_text_offset + 0.5) - device_text_offset;
                device_snap_offset += floor(device_glyph_offset + snap_bias) - device_glyph_offset;
                device_snap_offset = mat2(transform.m) * device_snap_offset;
                device_snap_offset = device_snap_offset / transform.m[3].w;
                break;
            }
        }
    }
    ...
    vec2 final_offset = device_snap_offset - task.content_origin + task.common_data.task_rect.p0;
