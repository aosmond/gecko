...
template <typename i32x4_t, typename i16x8_t, typename u16x8_t, typename u8x16_t>
static already_AddRefed<DataSourceSurface> ApplyColorMatrix_SIMD(
  ...
  for (int32_t y = 0; y < size.height; y++) {
    for (int32_t x = 0; x < size.width; x += 4) {
      ...
      p1234 = simd::Load8<u8x16_t>(&sourceData[sourceIndex]);
      ...
      p1234 = simd::PackAndSaturate16To8(p12, p34);
      ...
      simd::Store8(&targetData[targetIndex], result);
    }
  }
