...
DrawTargetRecording::DrawTargetRecording(
  ...
  printf_stderr(
      "[AO] [%p] DrawTargetRecording::DrawTargetRecording -- canvas\n", this);
...
DrawTargetRecording::DrawTargetRecording(DrawEventRecorder* aRecorder,
    ...
    : mRecorder(static_cast<DrawEventRecorderPrivate*>(aRecorder)),
      ...
      mRect(aRect) {
  printf_stderr(
      "[AO] [%p] DrawTargetRecording::DrawTargetRecording -- recorder\n", this);
...
DrawTargetRecording::DrawTargetRecording(const DrawTargetRecording* aDT,
  ...
  printf_stderr(
      "[AO] [%p] DrawTargetRecording::DrawTargetRecording -- target\n", this);
  mFormat = aFormat;
  DrawTarget::SetPermitSubpixelAA(IsOpaque(mFormat));
}
...
void DrawTargetRecording::FlushTransform() {
  if (!mRecordedTransform.ExactlyEquals(mTransform)) {
    ...
    printf_stderr("[AO] [%p] DrawTargetRecording::%s -- [%f %f; %f %f; %f %f]\n",
                  this, __func__, transform._11, transform._12, transform._21,
...
void DrawTargetRecording::FlushItem(const IntRect& aBounds) {
  ...
  mRecorder->SetCurrentDrawTarget(this);
  // We know that a newly created DrawTarget will have an identity transform,
  // so we can avoid writing it again if they match. Defer writing until the
