...
void gfxFontCache::AddNew(gfxFont* aFont) {
  ...
  nsTArray<RefPtr<gfxFont>> discard;
  {
    ...
    // We should never be replacing an existing font.
    if (entry->mFont) {
      MOZ_ASSERT_UNREACHABLE("Replacing existing font?");
      if (entry->mFont == aFont) {
        MarkUsedLocked(aFont, lock);
        return;
      } else {
        RemoveObjectLocked(entry->mFont, lock);
        discard.AppendElement(std::move(entry->mFont));
      }
    }
    entry->mFont = aFont;
