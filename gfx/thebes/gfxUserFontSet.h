...
class gfxUserFontEntry : public gfxFontEntry {
 ...
 public:
  ...
  // Get a copy of the attributes of the entry. This allows one to query the
  // values without continuing to hold the lock.
  void GetAttributes(gfxUserFontAttributes& aAttr) const;
  ...
  mozilla::StyleFontDisplay GetFontDisplay() const {
 ...
 protected:
  ...
  mozilla::StyleFontDisplay mFontDisplay
      MOZ_GUARDED_BY(mLock);  // timing of userfont fallback
