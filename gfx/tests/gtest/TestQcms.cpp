...
static size_t GetRgbInputBufferImpl(UniquePtr<uint8_t[]>& aOutBuffer) {
  ...
  const size_t pixelSize = kHasAlpha ? 4 : 3;
  const size_t pixelCount = colorSampleMax * colorSampleMax * 255 * 3;
  ...
  for (size_t r = 0; r < 255; ++r) {
    for (size_t g = 0; g < colorSampleMax; ++g) {
      for (size_t b = 0; b < colorSampleMax; ++b) {
        color[kRIndex] = r;
        color[kGIndex] = colorSamples[g];
        color[kBIndex] = colorSamples[b];
        if (kHasAlpha) {
          color[kAIndex] = 0x80;
        }
        color += pixelSize;
  ...
  for (size_t r = 0; r < colorSampleMax; ++r) {
    for (size_t g = 0; g < 255; ++g) {
      for (size_t b = 0; b < colorSampleMax; ++b) {
        color[kRIndex] = colorSamples[r];
        color[kGIndex] = g;
        color[kBIndex] = colorSamples[b];
        if (kHasAlpha) {
          color[kAIndex] = 0x80;
