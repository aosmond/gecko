...
static void TakeVideoFrameRequestCallbacksFrom(
  ...
  nsTArray<RefPtr<HTMLVideoElement>> videoCallbacks;
  aDocument->TakeVideoFrameRequestCallbacks(videoCallbacks);
  if (!videoCallbacks.IsEmpty()) {
    aTarget.AppendElement(aDocument);
    aTarget.LastElement().mVideoCallbacks = std::move(videoCallbacks);
  }
}
...
static void TakeFrameRequestCallbacksFrom(
  ...
  aDocument->TakeFrameRequestCallbacks(callbacks);
  if (!callbacks.IsEmpty()) {
    aTarget.AppendElement(aDocument);
    aTarget.LastElement().mCallbacks = std::move(callbacks);
  }
}
...
void nsRefreshDriver::RunFrameRequestCallbacks(TimeStamp aNowTime) {
  ...
  // First grab the throttled and unthrottled video frame request callbacks as
  // they need to be processed first.
  for (Document* doc : throttledFrameRequestCallbackDocs) {
    TakeVideoFrameRequestCallbacksFrom(doc, frameRequestCallbacks);
  }
  ...
  if (!frameRequestCallbacks.IsEmpty()) {
    ...
    frameRequestCallbacks.Clear();

    // Second grab the throttled and unthrottled frame request callbacks. There
    // may be new entries due to the video frame request callbacks above.
    for (Document* doc : throttledFrameRequestCallbackDocs) {
