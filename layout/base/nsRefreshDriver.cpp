...
// TODO: AZ NOTES
void nsRefreshDriver::RevokeVideoFrameRequestCallbacks(mozilla::dom::HTMLVideoElement* aElement) {
...
void nsRefreshDriver::ScheduleVideoFrameRequestCallbacks(mozilla::dom::HTMLVideoElement* aElement) {
  printf_stderr("AZ: %s:%s:%d appending element to list of video frame callbacks\n",
                __FILE__, __func__, __LINE__);
  ...
  if (regSched || thrSched) {
    printf_stderr("AZ: %s:%s:%d VFC double schedule attempt! reg = %d , thr = %d, shouldThrottle = %d\n",
                  __FILE__, __func__, __LINE__,
                  regSched, thrSched, shouldThrottle);

    if (!regSched && thrSched && !shouldThrottle) {
      printf_stderr("AZ: %s:%s:%d VFC double schedule attempt, move from throttled...\n",
                  __FILE__, __func__, __LINE__);
  ...
  if (aElement->MozPaintedFrames() == 0
      || shouldThrottle) {
    mThrottledVideoFrameRequestCallbackElems.AppendElement(aElement);
    printf_stderr("AZ: %s:%s:%d Added throttled VideoFrameRequest (now have: %zu)\n",
                  __FILE__, __func__, __LINE__,
                  mThrottledVideoFrameRequestCallbackElems.Length());
  } else {
    mVideoFrameRequestCallbackElems.AppendElement(aElement);
    printf_stderr("AZ: %s:%s:%d Added UN-throttled VideoFrameRequest (now have: %zu)\n",
                  __FILE__, __func__, __LINE__,
...
void nsRefreshDriver::RunVideoFrameRequestCallbacks(TimeStamp aNowTime) {
  ...
  To run the video frame request callbacks for a HTMLVideoElement video with a timestamp now, run the following steps:
    ...
    2. Let metadata be the VideoFrameCallbackMetadata dictionary built from video’s latest presented frame.
    ...
    8. For each entry in callbacks
        ...
        i.   If the entry’s canceled boolean is true, continue to the next entry.
  ...
  */
  ...
  // Grab all of our frame request callbacks up front.
  nsTArray<HTMLVideoElementCallbacks> videoFrameRequestCallbacks(mVideoFrameRequestCallbackElems.Length() + mThrottledVideoFrameRequestCallbackElems.Length());

  nsTArray<HTMLVideoElement*> dupeCheck( mVideoFrameRequestCallbackElems.Length() + mThrottledVideoFrameRequestCallbackElems.Length());
  ...
  if (!tickThrottledFrameRequests && aNowTime >= mNextThrottledFrameRequestTick) {
    mNextThrottledFrameRequestTick = aNowTime + mThrottledFrameRequestInterval;
    tickThrottledFrameRequests = true;
  }
  printf_stderr("AZ: %s:%s:%d tickThrottledFrameRequests: %d\n", __FILE__, __func__, __LINE__, tickThrottledFrameRequests);

  for (HTMLVideoElement* el : mThrottledVideoFrameRequestCallbackElems) {
    MOZ_RELEASE_ASSERT(el); Document* doc = el->OwnerDoc(); MOZ_RELEASE_ASSERT(doc);
    if (
        false
        || el->GetMediaInfo().mVideo.mDisplay.Width() <= 0
    ...
    if (tickThrottledFrameRequests) {
      ...
      printf_stderr("AZ: %s:%s:%d ticking and REMOVING UN-throttled element...\n", __FILE__, __func__, __LINE__);
  ...
  for (HTMLVideoElement* el : mVideoFrameRequestCallbackElems) {
    ...
    if (
        ...
        || el->GetPlayedOrSeeked() == 0
        || el->MozPaintedFrames() == 0
        || !el->HasVideo()
       ){
      printf_stderr("AZ: %s:%s:%d get VIDEO ELEMENT NOT READY, RE-ADDING CALLBACK!", __FILE__, __func__, __LINE__);
    ...
    TakeVideoFrameRequestCallbacksFrom(el, videoFrameRequestCallbacks);
    MOZ_RELEASE_ASSERT(videoFrameRequestCallbacks.Length() > 0, "Retreived callbacks list length should not be zero!");
  }

  printf_stderr("AZ: %s:%s:%d [t], (m)VideoFrameRequestCallbackElems is size: [%zu] (%zu) %zu, clearing mV\n", __FILE__, __func__, __LINE__,
                mThrottledVideoFrameRequestCallbackElems.Length(),
                mVideoFrameRequestCallbackElems.Length(),
  ...
  if (!videoFrameRequestCallbacks.IsEmpty()) {
    for (const HTMLVideoElementCallbacks& elemCallbacks : videoFrameRequestCallbacks) {
      TimeStamp startTime = TimeStamp::Now();
      ...
      if (
          ...
          doc->NotifyVideoFrameCallbacks(el);
          MOZ_RELEASE_ASSERT(false, "callback shouldn't be broken at this point.");
          continue;
      }
        ...
        printf_stderr("%s:%s:%d AZ - EXECUTING REQUESTVIDEOFRAMECALLBACK CALLBACK\n", __FILE__, __func__, __LINE__);

        LogVideoFrameRequestCallback::Run run(callback.mCallback);
        MOZ_KnownLive(callback.mCallback)->Call(timeStamp, *(el->GetVideoFrameCallbackMetadata()));
      }
    }
  }

  printf_stderr("AZ: %s:%s:%d [t], LEAVING RVFC FUNC, (m)VideoFrameRequestCallbackElems is size: [%zu] (%zu) %zu, clearing mV\n", __FILE__, __func__, __LINE__,
                mThrottledVideoFrameRequestCallbackElems.Length(),
                mVideoFrameRequestCallbackElems.Length(),
                videoFrameRequestCallbacks.Length()
               );
