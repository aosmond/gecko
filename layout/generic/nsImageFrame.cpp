...
void nsImageFrame::DisplayAltText(nsPresContext* aPresContext,
                                  ...
                                  const nsRect& aRect) {
  // Set font and color
  aRenderingContext.SetColor(sRGBColor::FromABGR(StyleText()->mColor.ToColor()));
...
static void PaintDebugImageMap(nsIFrame* aFrame, DrawTarget* aDrawTarget,
  ...
  f->GetImageMap()->Draw(
      aFrame, *aDrawTarget,
      ColorPattern(ToDeviceColor(sRGBColor(0.f, 0.f, 0.f, 1.f))));
}
#endif
...
ImgDrawResult nsImageFrame::PaintImage(gfxContext& aRenderingContext,
  ...
  if (nsImageMap* map = GetImageMap()) {
    ...
    // solid white stroke:
    ColorPattern white(ToDeviceColor(sRGBColor(1.f, 1.f, 1.f, 1.f)));
    map->Draw(this, *drawTarget, white);

    // then dashed black stroke over the top:
    ColorPattern black(ToDeviceColor(sRGBColor(0.f, 0.f, 0.f, 1.f)));
