  ...
  for (int i = aMaskFrames.Length() - 1; i >= 0; i--) {
    ...
    if (maskFrame) {
      ...
      if (svgMask) {
        ...
        aMaskDT->MaskSurface(ColorPattern(DeviceColor(0.0, 0.0, 0.0, 1.0)), svgMask,
...
bool nsSVGIntegrationUtils::PaintMask(const PaintFramesParams& aParams) {
  ...
  if (maskUsage.shouldApplyBasicShapeOrPath) {
    ...
    if (!maskUsage.shouldGenerateMaskLayer) {
      ...
      // opaque white.
      // FIXME(aosmond): should this actually use color management?!
      ctx.SetDeviceColor(DeviceColor(1.0, 1.0, 1.0, 1.0));
      ctx.Fill();
