...
void FontFaceSet::Add(FontFace& aFontFace, ErrorResult& aRv) {
  ...
  if (NS_WARN_IF(!fontImpl)) {
    aRv.ThrowInvalidStateError("FontFace has been destroyed");
    return;
  }
...
bool FontFaceSet::HasAvailableFontFace(FontFace* aFontFace) {
  FontFaceImpl* fontImpl = aFontFace->GetImpl();
  return fontImpl ? fontImpl->IsInFontFaceSet(mImpl) : false;
}
...
void FontFaceSet::DidRefresh() {
  mImpl->CheckLoadingFinished();
}
